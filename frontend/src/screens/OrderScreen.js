...
export default function OrderScreen() {
  ...
  useEffect(() => {
    const onApproveVnPay = async () => {
      try {
        const { data } = await axios.put(`/api/orders/${order._id}/pay`, null, {
    ...
    if (paymentState && order._id) {
      console.log(paymentState);
      onApproveVnPay();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [paymentState, order._id]);
  ...
  return loading ? (
    ...
    <div>
      <Helmet>
        <title>Order {order.customId}</title>
      </Helmet>

      <h1 className="my-3">Order {order.customId}</h1>
      <Row>
        <Col md={8}>
          <Card className="mb-3">
            <Card.Body>
